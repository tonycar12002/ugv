...
from robotx_msgs.msg import Waypoint, WaypointList, roboteq_drive, UsvDrive
...
class PathPlanning(object):
	def __init__(self):
		...
		self.pub_back = rospy.Publisher("/cmd_drive", roboteq_drive, queue_size=1)
	...
	def call_back(self, msg):
		...
		if self.get_goal and not self.backward:
			if not self.callc:
				...
				self.callc = True
				#if self.state == 1 and not self.first_get_time and dis < 2.5:
			if not self.first_get_time and dis < 2.5:
				self.start_t = rospy.get_time()
				self.first_get_time = True
			self.end_t = rospy.get_time()
			print('Time: ', self.end_t-self.start_t)
			if (self.end_t-self.start_t) >= 21 and dis < 2.5:
				self.backward = True
