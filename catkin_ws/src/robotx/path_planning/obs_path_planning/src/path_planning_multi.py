...
class PathPlanning(object):
	def __init__(self):
		...
		self.start_obs = rospy.Service("/start_obs", Trigger, self.start_obs)
	...
	def call_new_goal(self, p):
		if self.new_series_goals:
			self.goal_point = []
		goal_point = self.safe_goal_point([p.pose.position.x, p.pose.position.y])
	...
	def new_goal(self, req):
		if self.new_series_goals:
			self.goal_point = []
		goal_point = self.safe_goal_point([req.waypointx, req.waypointy])
		self.goal_point.append(goal_point)
		...
		self.arrived = False
		self.state = 0
		print "Add New Goal: ", self.goal_point
	...
	def arrive_goal(self):
		...
		if dis <= self.arrive_dis:
			if not self.arrived:
				print "Arrived goal point: ", self.goal_point[-1]
			return True
		return False
	...
	def check_collision(self, obs):
		...
		if count == 1:
			...
			if self.distance(self.gate_point1, self.gate_point2) > 2*obs_size + self.cluster_dis:
				print(self.gate_point1, self.gate_point2)
				self.is_gate = True
			print('In obs')
		return collision
	...
	def find_way(self, obs):
		...
		self.key2 = p2_shift
		#self.danger_point = False
		if self.first or self.danger_point:
		...
		if self.is_gate:
			case1 = self.gate_point1 == obs1 and self.gate_point2 == obs2
			case2 = self.gate_point1 == obs2 and self.gate_point2 == obs1
			if case1 or case2:
				print('Gate!!!!!')
