...
class PathPlanning(object):
	def __init__(self):
		...
		self.arrived = False
		self.safe_dis = rospy.get_param("~safe_dis",3.8)
		self.init_param()
	...
	def shift_point(self, dis, m, d, p, p_now):
		...
		angle1 = self.get_angle(p_now, new_point1, p)
		angle2 = self.get_angle(p_now, new_point2, p)
		if self.p_now == self.start_point:
			if self.first_angle == 1:
				if angle1 > 0:
					return new_point1
				elif angle2 > 0:
					return new_point2
			elif self.first_angle == -1:
				if angle1 < 0:
					return new_point1
				elif angle2 < 0:
					return new_point2
	...
	def find_shift_point(self, p_now, p_next, dis, p_new, move):
		...
		if move:
			d = -d
			dis = abs(dis)
		return self.shift_point(dis, m, d, p_new, p_now)
	...
	def find_way_first(self, obs_vertex):
		obs_angle_list = []
		angle_list_positive = []
		angle_list_negetive = []
		chosen_angle = None
	...
	def find_way(self, obs_vertex):
		...
		for i in range(len(obs_vertex)):
			...
			if dis < nearest_dis:
				...
				nearest_p = obs_vertex[i]
				if self.p_now == self.start_point:
					nearest_angle = self.angle_from_robot(obs_vertex[i])
				else:
					nearest_angle = self.get_angle(self.p_now, self.p_next, obs_vertex[i])
			if self.p_now == self.start_point:
				obs_angle_list.append([obs_vertex[i], self.angle_from_robot(obs_vertex[i])])
			else:
				obs_angle_list.append([obs_vertex[i], self.get_angle(self.p_now, self.p_next, obs_vertex[i])])
			if obs_angle_list[i][1] >= 0:
				angle_list_positive.append(obs_angle_list[i][1])
		...
		if len(angle_list_negetive) == 0:
			self.first_angle = 1
			abs_angle = []
			for i in angle_list_positive:
				abs_angle.append(abs(i))
			abs_angle.sort()
			chosen_angle = abs_angle[0]
			for i in range(len(obs_vertex)):
				if abs(obs_angle_list[i][1]) == chosen_angle:
					p_new = obs_angle_list[i][0]
					break
			return p_new, chosen_angle
	...
	def _process(self):
		...
		for obs_index in range(self.obstacle_list.size):
			...
			if wrong_waypoint:
				bug = self.add_obstacle(self.pre_obs_idx, obs_index)
				#self.pre_obs_idx = obs_index
				if bug and self.p_now == self.start_point:
					#self.process_obs_list.append([obs_index])
					if [obs_index] in self.process_obs_list:
						self.p_now = self.wp_list[-2]
						self.p_next = self.wp_list[-1]
						self.wp_index = self.wp_list.index(self.p_now)
						return
					else:
