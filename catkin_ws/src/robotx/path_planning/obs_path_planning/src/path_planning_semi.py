...
class PathPlanning(object):
	def __init__(self):
		...
		self.arrived = False
		self.count_goal_num = 0
		self.safe_dis = rospy.get_param("~safe_dis",3.45)
		self.cluster_dis = 2.*self.safe_dis + 0.9
		self.init_param()
	...
	def new_goal(self, req):
		if self.count_goal_num > 3:
			self.safe_dis = 3.45
		else:
			self.safe_dis = 3.45
		#print('Safe Dis: ', self.safe_dis)
		...
		self.arrived = False
		self.state = 0
		self.count_goal_num = self.count_goal_num + 1
	...
	def find_way(self, obs):
		...
		#print(first_dis)
		if (self.first or self.danger_point) and first_dis < 3.*self.safe_dis:
